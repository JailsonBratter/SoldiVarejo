...
{
    public partial class SolicitacaoDeEncomendaDetalhes : visualSysWeb.code.PagePadrao
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (usr != null)
            {
                if (Request["novo"] != null)
                {
                    if (!IsPostBack)
                    {
                        solicitacao_producaoDAO obj = new solicitacao_producaoDAO(usr,1);
                ...
                else
                {
                    if (Request["codigo"] != null)
                    {
                        if (!IsPostBack)
                        {
                            ...
                            status = "visualizar";
                            solicitacao_producaoDAO obj = new solicitacao_producaoDAO(cod,1, usr);
                            Session.Remove("objSolicita" + urlSessao());
                            Session.Add("objSolicita" + urlSessao(), obj);
                            carregarDados();
        ...
        private void atualizaItens(GridView grid)
        {
            ...
            for (int i = 0; i < grid.Rows.Count; i++)
            {
                ...
                {
                    TextBox txtObs = (TextBox)grid.Rows[i].FindControl("txtObs");
            ...
            try
            {
                ...
                while (rs.Read())
                {
                    if (!obj.itemIncluido(rs["PLU"].ToString()))
                    {
                        ...
                        solicitacao_producao_itensDAO objItem = new solicitacao_producao_itensDAO(usr,1);
        ...
        {
            Response.Redirect("SolicitacaoDeEncomenda.aspx");
        }
