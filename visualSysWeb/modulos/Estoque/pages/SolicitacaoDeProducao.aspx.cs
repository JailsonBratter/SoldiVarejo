...
{
    public partial class SolicitacaoDeProducao : visualSysWeb.code.PagePadrao
    {
        ...
        private void pesquisa(bool limitar)
        {
            ...
            String sql = "Select Codigo,Descricao,convert(varchar,Data_cadastro,103) as data_cadastro,usuario_cadastro,status" +
                 ",STUFF ( " +
                   " ( " +
                   " Select  ', ' + pedido from solicitacao_producao_Pedidos where tipo_producao = 0 and filial = solicitacao_producao.filial and codigo = solicitacao_producao.codigo " +
                   " FOR XML PATH('') " +
                   " ) " +
                    ",1,2,''" +
               " ) as pedido" +
               "  from solicitacao_producao";
            String sqlWhere = " isnull(tipo_producao,0) = 0";
