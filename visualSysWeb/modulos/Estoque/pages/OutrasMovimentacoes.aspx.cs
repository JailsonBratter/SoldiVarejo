...
namespace visualSysWeb.modulos.Estoque.pages
{
    public partial class OutrasMovimentacoes : visualSysWeb.code.PagePadrao     //inicio da classe 
    {
        ...
        static String sqlGrid = "select codigo_inventario,descricao_inventario,data=convert(varchar,data,103),usuario,status,tipoMovimentacao, ISNULL(CONVERT(VARCHAR, DATAHORA_ENCERRAMENTO, 102) + ' ' + CONVERT(VARCHAR, DATAHORA_ENCERRAMENTO, 108), '') AS DataHora_Encerramento from inventario";//colocar os campos no select que ser?o apresentados na tela
        ...
        {
            Response.Redirect("~/modulos/Estoque/pages/OutrasMovimentacoesDetalhes.aspx?novo=true"); // colocar caminho da pagina de Detalhes verificar Case sensitive
        }
        private void pesquisar(bool limitar)
        {
            ...
            try
            {
                ...
                sql += " inventario.filial='" + usr.getFilial() + "'";

                if (!sql.Equals(""))
                {
                    tb = Conexao.GetTable(sqlGrid + " where " + sql + " order by CONVERT(varchar,data,102) desc, inventario.Codigo_Inventario desc ", usr, limitar);
                }
                else
                {
                    tb = Conexao.GetTable(sqlGrid + " order by CONVERT(varchar,data,102) desc , inventario.Codigo_Inventario desc", usr, false);
                }
        ...
        protected void gridPesquisa_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (int.TryParse(e.CommandArgument.ToString(), out index))
            {
                ...
                {
                    HyperLink mOM = (HyperLink)gridPesquisa.Rows[index].Cells[0].Controls[0];
                    String strOutra = mOM.Text;
                    Response.Redirect("~/modulos/notafiscal/pages/NfSaidaDetalhes.aspx?tela=NF002&novo=true&pedidoImporta=" + strOutra);
        ...
        {
            // = (RadioButton)e.Row.FindControl("RdoListaItem");
