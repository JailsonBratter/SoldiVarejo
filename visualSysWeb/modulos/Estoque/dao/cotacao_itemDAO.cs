...
namespace visualSysWeb.dao
{
    public class cotacao_itemDAO
    {
        ...
        public Decimal preco_compra = 0;


        public cotacao_itemDAO(User usr)
        {
            this.usr = usr;
        }
        public cotacao_itemDAO(String cotacao, String plu, User usr)
        { //colocar campo index da tabela
            this.usr = usr;
            String sql = "Select * , mercadoria.descricao , mercadoria.preco_compra from  cotacao_item inner join mercadoria on cotacao_item.mercadoria = mercadoria.plu  where cotacao =" + cotacao + " and mercadoria='" + plu + "'";
        ...
        public void carregarDados(SqlDataReader rs)
        {
            if (rs.Read())
            {
                Vencedor = rs["Vencedor"].ToString();
                Mercadoria = rs["Mercadoria"].ToString();
                EAN = rs["EAN"].ToString();
                Quantidade = (Decimal)(rs["Quantidade"].ToString().Equals("") ? new Decimal() : rs["Quantidade"]);
        ...
        public ArrayList arrString()
        {
            ArrayList arr = new ArrayList();
            arr.Add(Vencedor.ToString());
            arr.Add(Mercadoria.ToString());
            arr.Add(EAN);
            arr.Add(Quantidade.ToString("N2"));
            arr.Add(Cotacao.ToString());
            arr.Add(Filial.ToString());
            arr.Add(embalagem.ToString("N2"));
            arr.Add(descricao);
            arr.Add(preco_compra.ToString("N2"));
            return arr;
        }
        private void update(SqlConnection cnn, SqlTransaction trans)
        {
            try
            {
                if (inserido)
                ...
                else
                {
                    ...
                    String sql = "update  cotacao_item set " +
                                  "Vencedor='" + Vencedor + "'" +
                                  ",Quantidade=" + Quantidade.ToString().Replace(",", ".") +

                                  ",embalagem=" + embalagem.ToString().Replace(",", ".") +

                        "  where cotacao =" + Cotacao + " and Mercadoria='" + Mercadoria + "' and Filial='" + Filial + "'";
        ...
        public bool salvar(bool novo, SqlConnection cnn, SqlTransaction trans)
        {
            if (novo)
            {
                insert(cnn, trans);
        ...
        public bool excluir(SqlConnection cnn, SqlTransaction trans)
        {
            String sql = "delete from cotacao_item  where cotacao =" + Cotacao + " and Mercadoria='" + Mercadoria + "'";
            Conexao.executarSql(sql, cnn, trans);
            return true;
        }
        ...
        public override bool Equals(Object obj)
        {
            try
            {
                cotacao_itemDAO objct = (cotacao_itemDAO)obj;
                if (this.Mercadoria.Equals(objct.Mercadoria))
                    return true;
                else
                    return false;
        ...
        private void insert(SqlConnection cnn, SqlTransaction trans)
        {
            try
            {
                String sql = " insert into cotacao_item (" +
                          "Vencedor," +
                          "Mercadoria," +
                          "Quantidade," +
                          "Cotacao," +
...
{ 
                  static DataTable tb;
                  static String sqlGrid = ""select * from cotacao_item";//colocar os campos no select que ser?o apresentados na tela
                  ...
                  {
                      Response.Redirect("~/modulos/nome Do Modulo/pages/ cotacao_itemDetalhes.aspx?novo=true"); // colocar caminho da pagina de Detalhes verificar Case sensitive
                  }
...
/*================================html tela de Pesquisa==========================================
   ...
   <center><h1>cotacao_item</h1></center>
    <hr />              
       ...
       <div class="filter" id="filtrosPesq" runat="server" visible="false">           
        ...
        <div class="gridTable">          
            <asp:GridView ID="gridPesquisa" runat="server" AutoGenerateColumns="False"           
                 ...
                 <Columns> 
                    <asp:HyperLinkField DataTextField="Vencedor" Text="Vencedor" Visible="true" 
                    HeaderText="Vencedor" DataNavigateUrlFormatString="~/modulos/Coloca Nome Do Modulo/cotacao_itemDetalhes.aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  

                    <asp:HyperLinkField DataTextField="Mercadoria" Text="Mercadoria" Visible="true" 
                    HeaderText="Mercadoria" DataNavigateUrlFormatString="~/modulos/Coloca Nome Do Modulo/cotacao_itemDetalhes.aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  

                    <asp:HyperLinkField DataTextField="Quantidade" Text="Quantidade" Visible="true" 
                    HeaderText="Quantidade" DataNavigateUrlFormatString="~/modulos/Coloca Nome Do Modulo/cotacao_itemDetalhes.aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  

                    <asp:HyperLinkField DataTextField="Cotacao" Text="Cotacao" Visible="true" 
                    HeaderText="Cotacao" DataNavigateUrlFormatString="~/modulos/Coloca Nome Do Modulo/cotacao_itemDetalhes.aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  

                    <asp:HyperLinkField DataTextField="Filial" Text="Filial" Visible="true" 
                    HeaderText="Filial" DataNavigateUrlFormatString="~/modulos/Coloca Nome Do Modulo/cotacao_itemDetalhes.aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  

                    <asp:HyperLinkField DataTextField="embalagem" Text="embalagem" Visible="true" 
                    HeaderText="embalagem" DataNavigateUrlFormatString="~/modulos/Coloca Nome Do Modulo/cotacao_itemDetalhes.aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  
...
using System.Data.SqlClient;
  ...
  {
                 protected static cotacao_itemDAO obj= null ;
                 static String camporeceber = "";
                 protected void Page_Load(object sender, EventArgs e)     
                 {
                      User usr = (User)Session["User"];
                      obj = new cotacao_itemDAO();
                           ...
                           if (Request.Params["campoIndex"] != null)  // colocar o campo index da tabela
                           {
                              try
                              {
                                   if (!IsPostBack)
                                   {
                                        ...
                                        status = "visualizar";
                                        obj = new cotacao_itemDAO(index,usr);
                                        carregarDados();
                                    }
      ...
      {
                                         txtVencedor.Text=obj.Vencedor.ToString();
                                         txtMercadoria.Text=obj.Mercadoria.ToString();
                                         txtQuantidade.Text=string.Format("{0:0,0.00}",obj.Quantidade);
     ...
     {
                                         obj.Vencedor=txtVencedor.Text;
                                         obj.Mercadoria=txtMercadoria.Text;
                                         obj.Quantidade=(txtQuantidade.Text.Equals("")?0:Decimal.Parse(txtQuantidade.Text));
...
<div class="cabMenu">                  
       <center> <h1>Detalhes do cotacao_item</h1></center>                  
</div>                  
    <asp:Panel ID="pnBtn" runat="server" CssClass="cabMenu">                  
    </asp:Panel>                  
    <br />              
...
/*--Campos Form
                                      <td >                   <p>Vencedor</p>
                   <asp:TextBox ID="txtVencedor" runat="server" ></asp:TextBox>
                   </td>
                   ...
                   <asp:TextBox ID="txtMercadoria" runat="server" ></asp:TextBox>
                                      ...
                                      <td >                   <p>Quantidade</p>
                   <asp:TextBox ID="txtQuantidade" runat="server" CssClass="numero" ></asp:TextBox>
                   </td>
