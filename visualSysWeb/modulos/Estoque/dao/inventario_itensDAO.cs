    ...
    public class inventario_itensDAO
    {
        ...
        public String departamento
        {
            get
            {
                try
                {
                    if (!strdepartamento.Equals(""))
                    {
                        return strdepartamento;
                    }
                    else
                    {
                        strdepartamento = Conexao.retornaUmValor("select Departamento.Descricao_departamento from mercadoria inner join Departamento on mercadoria.Codigo_departamento = Departamento.Codigo_departamento where mercadoria.PLU='"+PLU+"'", null).Trim();
                        return strdepartamento;
                    }
                }
                catch (Exception)
        ...
        public Decimal vContada = 0.000M;
        public Decimal Contada 
        {
            get{ return vContada;}
            set{
                if(!value.Equals(vContada))
                {
                    alterado = true;
                    vContada = value;
                }
        ...
        public ArrayList ArrToString()
        {
            ...
            item.Add(INDEX.ToString());
            item.Add(PLU.Trim());
            item.Add(EAN.Trim());
            item.Add(Referencia.Trim());
            item.Add(Descricao);
            item.Add(Codigo_inventario);
            item.Add(Saldo_atual.ToString());
            item.Add(Contada.ToString("N2"));
            item.Add(Custo.ToString("N2"));
            item.Add(Qtde.ToString("N3"));
            item.Add(Diferenca.ToString("N3"));
        ...
        public void carregarDados(SqlDataReader rs)
        {
            if (rs.Read())
            {
                ...
                Venda = (Decimal)(rs["Venda"].ToString().Equals("") ? new Decimal() : rs["Venda"]);
        ...
        {
            inventario_itensDAO obj = other as inventario_itensDAO;
            if (obj != null)
            {
                return this.PLU.Equals(obj.PLU);
            }
            else
            {
                return false;
