...
{
    public partial class PedidoVenda :visualSysWeb.code.PagePadrao     //inicio da classe 
    {
        static DataTable tb;
        static String sqlGrid = "select pedido,simples= case when pedido_simples=1 then 'SIM' ELSE 'NAO' END, cliente_fornec,cliente.nome_cliente,Status = case when status=1 then 'ABERTO'else case " +
         " when status=2 then 'FECHADO'  else case when status=3 then 'CANCELADO'  else case when status=4 then 'PENDENTE ENTREGA' else case when status=4 then 'TRANSITO' else case when status=7 then 'LIBERADO' END end end END END END,convert(varchar,pedido.data_Cadastro,103) Data_Cadastro,Total from pedido inner join cliente on cliente.codigo_cliente = pedido.cliente_fornec where tipo=1";//colocar os campos no select que ser?o apresentados na tela
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (usr !=null && usr.adm("PD002"))
                {
                    ...
                    {
                        tb = Conexao.GetTable(sqlGrid + " AND cliente.grupo_empresa IN(" + usr.grupoClientes + ")  order by CONVERT(VARCHAR ,pedido.data_cadastro,102) desc, pedido.pedido desc ", usr, true);
                    }
                    else
                    {
                        tb = Conexao.GetTable(sqlGrid + " order by CONVERT(VARCHAR ,pedido.data_cadastro,102) desc, pedido.pedido desc ", usr, true);
                    }
        ...
        protected void pesquisar(string ordem)
        {
            ...
            try
            {
                ...
                if (!usr.grupoClientes.Equals(""))
                {
                    sqlGrid += " AND cliente.grupo_empresa IN(" + usr.grupoClientes + ")";
                }
        ...
        protected void gridPesquisa_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (int.TryParse(e.CommandArgument.ToString() , out index))
            {
                if (e.CommandName.Equals("EmitirNf"))
                {
                    ...
                    Response.Redirect("~/modulos/notafiscal/pages/NfSaidaDetalhes.aspx?tela=NF002&novo=true&pedidoImporta=" + strPed);
                ...
                else
                {
                    ...
                    RedirectNovaAba("~/modulos/notafiscal/pages/DanfeReport.aspx?cliente_Fornecedor=" + clienteFornecedor + "&" +
                                                                               "numero=" + numero + "&" +
                                                                               "tipoNf=1 &" +
                                                                               "tipoOrigem=P");
        ...
        protected void btnTransmitir_Click(object sender, EventArgs e)
        {
            ...
            {
                User usr = (User)Session["User"];
                KCWSPedido KCWSPed = new KCWSPedido("ListarNovos", usr);
                if(!KCWSPed.strError.Equals(""))
                {
                    throw new Exception(KCWSPed.strError);
                }
                pesquisar();
                lblPesquisaErro.Text = "Atualizado Com Sucesso as "+ DateTime.Now.ToString();
