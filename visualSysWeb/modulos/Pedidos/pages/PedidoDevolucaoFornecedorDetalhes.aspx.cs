...
{
    public partial class PedidoDevolucaoFornecedorDetalhes : visualSysWeb.code.PagePadrao
    {
                ...
                if (Request.Params["campoIndex"] != null)  // colocar o campo index da tabela
                {
                    try
                    {
                        if (!IsPostBack)
                        {
                            ...
                            status = "visualizar";
                            pedidoDAO obj = new pedidoDAO(index, 4, usr);
        ...
        private void novoPedido()
        {
            ...
            txthora.Text = "00:00";
            txtCFOP.Text = Funcoes.valorParametro("NAT_OP_DEV_FORNECEDOR", usr);
            txtCentroCusto.Text = Funcoes.valorParametro("CC_PEDIDO_DEV_FORNEC", usr);
            pedidoDAO obj = (pedidoDAO)Session["pedido" + Request.Url.ToString().Replace(" ", "").Replace("+", "").Replace("%20", "")];
        ...
        protected override void btnEditar_Click(object sender, EventArgs e)
        {
            if (!ddlStatus.SelectedValue.Equals("1"))
            {
                ...
                if (usr.adm(usr.tela))
                {
                    ...
                    txtNomeFornecedor.BackColor = System.Drawing.Color.White;
        ...
        {
            Response.Redirect("pedidoDevolucaoFornecedor.aspx");//colocar endereco pagina de pesquisa
        }
        ...
        private void carregarDados()
        {
            ...
            txtNomeFornecedor.Text = obj.NomeFornecedor;
        ...
        protected void carregarMercadorias()
        {
            ...
            User usr = (User)Session["user"];
            String sqlMercadoria = " select  s.PLU, Ref = m.Ref_fornecedor, m.Descricao, SUM(Qtde)Qtde,'1' as Embalagem, convert(numeric(12,2), (vlr/Qtde)) Vlr,s.Documento as Cupom , s.Caixa_saida as Pdv, convert(varchar,s.Data_movimento,103) as Data     " +
                                               //" from saida_estoque s inner join mercadoria m on s.PLU=m.PLU   LEFT OUTER JOIN tributacao d ON  S.nro_ECF = d.Nro_ECF  and d.Saida_ICMS = s.Aliquota_ICMS  " +
                                               ...
                                               ////" and m.descricao like '%" + txtDescricaoMercadoriaCupom.Text + "%'" +
            ...
            sqlMercadoria += " ORDER BY RTRIM(LTRIM(m.descricao))";
        ...
        protected void exibeLista()
        {
            ...
            {
                case "Fornecedor":
                    sqlLista = "select Fornecedor , Razao_social Nome, CNPJ from Fornecedor where Fornecedor like '%" + TxtPesquisaLista.Text + "%' or Razao_social like '%" + TxtPesquisaLista.Text + "%'"; ;
            ...
            if (!selecionado.Equals("") && !selecionado.Equals("------"))
            {
                try
                {
                    ...
                    if (listaAtual.Equals("Fornecedor"))
                    {
                        ...
                        txtNomeFornecedor.Text = ListaSelecionada(2);
                        pedidoDAO ped = (pedidoDAO)Session["pedido" + Request.Url.ToString().Replace(" ", "").Replace("+", "").Replace("%20", "")];
        ...
        protected void btnFecharMecadoria_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                if (selecionados != null && selecionados.Count > 1)
                {
                    ...
                    {
                        TextBox txtQtdeItem = (TextBox)sl.FindControl("txtQtd");
                        Label lblQtde = (Label)sl.FindControl("lblQTDPadrao");
                        Decimal tQtde = Decimal.Parse(txtQtdeItem.Text);
                        Decimal lQtde = Decimal.Parse(lblQtde.Text);
                        if (tQtde > lQtde)
                        {
                            txtQtdeItem.Focus();
                            txtQtdeItem.BackColor = System.Drawing.Color.Red;
                            throw new Exception("O Item " + sl.Cells[1].Text + " não pode ter a Quantidade maior que " + lQtde.ToString("N2"));
                        }
                    ...
                    foreach (GridViewRow sl in GridMercadoriaSelecionado.Rows)
                    {
                        ...
                        if (!txtQtdItem.Text.Equals("------"))
                        {
                            ...
                            TextBox txtEmbItem = (TextBox)sl.FindControl("txtEmbItem");
                            item.Embalagem = txtEmbItem.Text.Equals("") ? 1 : decimal.Parse(txtEmbItem.Text);
        ...
        protected void ImgBtnAddSelecionado_Click(object sender, ImageClickEventArgs e)
        {
            ...
            foreach (GridViewRow linha in gridMercadoria1.Rows)
            {
                ...
                if (rdo.Checked)
                {
                    ...
                    sel.Add(linha.Cells[4].Text);
                    sel.Add(linha.Cells[4].Text);
                    sel.Add(linha.Cells[5].Text);
                    sel.Add(linha.Cells[6].Text);
                    sel.Add(linha.Cells[6].Text);
                    sel.Add(linha.Cells[7].Text);
                    sel.Add(linha.Cells[8].Text);
                    sel.Add(linha.Cells[9].Text);
        ...
        protected void txtCliente_Fornec_TextChanged(object sender, EventArgs e)
        {
            if (!txtCliente_Fornec.Text.Trim().Equals(""))
            {
                pedidoDAO ped = (pedidoDAO)Session["pedido" + Request.Url.ToString().Replace(" ", "").Replace("+", "").Replace("%20", "")];
                ped.Cliente_Fornec = txtCliente_Fornec.Text;
                txtNomeFornecedor.Text = ped.NomeFornecedor;
        ...
        {
            String sql = "	Select Fornecedor +'|'+ rtrim(Razao_social) +'| CNPJ: '+isnull(CNPJ,'') from Fornecedor where fornecedor like '" + (prefixText.Length > 4 ? "%" : "") + prefixText + "%'";
        ...
        {
            if (txtNomeFornecedor.Text.IndexOf("|") >= 0)
            {
                String nome = txtNomeFornecedor.Text.Substring(txtNomeFornecedor.Text.IndexOf("|") + 1);
                txtCliente_Fornec.Text = txtNomeFornecedor.Text.Substring(0, txtNomeFornecedor.Text.IndexOf("|"));
                txtNomeFornecedor.Text = nome.Substring(0, nome.IndexOf("|"));
                //txtCodTbPreco.Focus();
        ...
        protected void btnImprimirTitulos_Click(object sender, ImageClickEventArgs e)
        {
            ...
            cliente.Add(txtNomeFornecedor.Text);
            titulos.Add(cliente);
