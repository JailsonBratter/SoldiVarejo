...
namespace visualSysWeb.modulos.Cadastro.pages
{
    ...
    {
        static DataTable tb;
        static String sqlGrid = "SELECT CASE WHEN ISNULL(Inativo, 0) = 0 THEN 'ATIVO' ELSE 'INATIVO' END AS Situacao , * FROM cartao";//colocar os campos no select que ser?o apresentados na tela
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                DllStatus.SelectedValue = "Ativo";
                Pesquisa();
            }
            pesquisar(pnBtn);
        }
        ...
        private void Pesquisa()
        {
            ...
            if (!DllStatus.SelectedValue.Equals("TODOS") && !DllStatus.SelectedValue.Equals(""))
            {
                if (!sql.Equals(""))
                {
                    sql += " and ";
                }
                sql += " ISNULL(Inativo, 0) = " + DllStatus.SelectedValue;
            }
            try
            {
        ...
        protected void gridPesquisa_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                if (e.Row.Cells[6].Text.ToUpper().Equals("INATIVO"))
                {
