  ...
  public class grupo_ccDAO
   {
         ...
         public grupo_ccDAO(){ }
         public grupo_ccDAO(String codGrupo,User usr){ //colocar campo index da tabela
             String sql="Select * from  grupo_cc where codigo_grupo ='"+ codGrupo+"' and Filial='MATRIZ'"  ;
        ...
        private void update(){
        try{
         String sql= "update  grupo_cc set "+
                       "Descricao_grupo='"+Descricao_grupo+"'"+
                       ",modalidade='"+modalidade+"'"+
             "  where codigo_grupo='" + codigo_grupo + "' and Filial='MATRIZ'";
         ...
         public bool salvar(bool novo) {
                 ...
                 Conexao.executarSql("UPDATE Centro_Custo SET modalidade = '" + modalidade + "' WHERE Centro_Custo.Codigo_centro_custo IN (SELECT Centro_Custo.Codigo_centro_custo FROM Centro_Custo INNER JOIN Subgrupo_CC ON Centro_Custo.Codigo_subgrupo= Subgrupo_CC.Codigo_SubGrupo INNER JOIN Grupo_CC ON Subgrupo_CC.codigo_grupo = Grupo_CC.codigo_grupo WHERE Grupo_CC.codigo_grupo =" + codigo_grupo + ")");  
                  return true;
         }
         
         public bool excluir() {

             int cont = Conexao.countSql("select codigo_grupo from subgrupo_cc where codigo_grupo='" + codigo_grupo + "'", null);
             if (cont > 0)
             {
                 throw new Exception("NÃO É PERMITIDO EXCLUIR UM GRUPO QUE CONTENHA SUBGRUPOS CADASTRADOS");
             }
             String sql = "delete from grupo_cc  where codigo_grupo= '" + codigo_grupo + "' and Filial='MATRIZ'" ; //colocar campo index
             Conexao.executarSql(sql);
             return true;
         }
     ...
     private void insert(){
         try{
             String sql = " insert into grupo_cc ("+
                       "codigo_grupo,"+
                       "Filial,"+
                       "Descricao_grupo,"+
                       "modalidade"+
...
{ 
                  static DataTable tb;
                  static String sqlGrid = ""select * from grupo_cc";//colocar os campos no select que ser?o apresentados na tela
                  ...
                  {
                      Response.Redirect("~/modulos/nome Do Modulo/pages/ grupo_ccDetalhes.aspx?novo=true"); // colocar caminho da pagina de Detalhes verificar Case sensitive
                  }
...
using System.Data.SqlClient;
  ...
  {
                 protected static grupo_ccDAO obj= null ;
      ...
      {
                                         txtcodigo_grupo.Text=obj.codigo_grupo.ToString();
     ...
     {
                                         obj.codigo_grupo=txtcodigo_grupo.Text;
...
<div class="cabMenu">                  
       <center> <h1>Detalhes do grupo_cc</h1></center>                  
</div>                  
    <asp:Panel ID="pnBtn" runat="server" CssClass="cabMenu">                  
    </asp:Panel>                  
    <br />              
...
/*--Campos Form
                                      <td >                   <p>codigo_grupo</p>
                   <asp:TextBox ID="txtcodigo_grupo" runat="server" ></asp:TextBox>
                   </td>
                   ...
                   <asp:TextBox ID="txtFilial" runat="server" ></asp:TextBox>
                                      ...
                                      <td >                   <p>Descricao_grupo</p>
                   <asp:TextBox ID="txtDescricao_grupo" runat="server" ></asp:TextBox>
                   </td>
