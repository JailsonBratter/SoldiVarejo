...
namespace visualSysWeb.dao
{
    ...
    {
        public int codigo_linha = 0;
        public String descricao_linha = "";
        public ArrayList cores = new ArrayList();


        public linhaDAO() { }
        public linhaDAO(String CodigoLinha, User usr)
        { //colocar campo index da tabela
            String sql = "Select * from  linha where codigo_linha =" + CodigoLinha;
            SqlDataReader rs = Conexao.consulta(sql, usr, true);
            carregarDados(rs);
        }

        public void addCor(cor_linhaDAO cor)
        {
            cor.codigo_linha = codigo_linha;
            cores.Add(cor);
        }
        public void removeCor(cor_linhaDAO cor)
        {
            cores.Remove(cor);
        }
        public void removeCor(int cor)
        {
            cores.RemoveAt(cor);
        }

        public DataTable CoresLinha()
        {
            ArrayList Cabecalho = new ArrayList();
            ArrayList itensCores = new ArrayList();
            Cabecalho.Add("Codigo_cor");
            Cabecalho.Add("Codgio_linha");
            Cabecalho.Add("Descricao_Cor");
            itensCores.Add(Cabecalho);
            if (cores != null && cores.Count > 0)
            {
                foreach (cor_linhaDAO cor in cores)
                {
                    itensCores.Add(cor.ArrToString());
                }
            }
            return Conexao.GetArryTable(itensCores);
        }
        ...
        public void carregarDados(SqlDataReader rs)
        {
            if (rs.Read())
            {
                codigo_linha = (rs["codigo_linha"] == null ? 0 : int.Parse(rs["codigo_linha"].ToString()));
                descricao_linha = rs["descricao_linha"].ToString();
                cores = cor_linhaDAO.arrayCoresLinha(codigo_linha);
            }
        ...
        public bool salvar(bool novo)
        {
            ...
            try
            {
                ...
                Conexao.executarSql("Delete from cor_linha where codigo_linha=" + codigo_linha, conn, tran);
                foreach (cor_linhaDAO cor in cores)
                {
                    cor.salvar(true, conn, tran);
                }
                tran.Commit();
            }
            catch (Exception err)
        ...
        public bool excluir()
        {
            ...
            String sql = "delete from linha  where codigo_linha= " + codigo_linha; //colocar campo index
            Conexao.executarSql(sql);
            Conexao.executarSql("Delete from cor_linha where codigo_linha=" + codigo_linha);
...
{ 
                  static DataTable tb;
                  static String sqlGrid = ""select * from linha";//colocar os campos no select que ser?o apresentados na tela
                  ...
                  {
                      Response.Redirect("~/modulos/nome Do Modulo/pages/ linhaDetalhes.aspx?novo=true"); // colocar caminho da pagina de Detalhes verificar Case sensitive
                  }
       ...
       <div class="filter" id="filtrosPesq" runat="server" visible="false">           
        ...
        <div class="gridTable">          
            <asp:GridView ID="gridPesquisa" runat="server" AutoGenerateColumns="False"           
                 AllowPaging="True" 
                 PageSize="20"  
                 onpageindexchanging="gridPesquisa_PageIndexChanging" CellPadding="2"  
                 ForeColor="#333333" GridLines="None"  
                 > 
                 <AlternatingRowStyle BackColor="#BEBEBE" ForeColor="#284775" /> 
                 <Columns> 
                    <asp:HyperLinkField DataTextField="codigo_linha" Text="codigo_linha" Visible="true" 
                    HeaderText="codigo_linha" DataNavigateUrlFormatString="~/modulos/colocar endere?o da pagina de detalhes .aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  

                    <asp:HyperLinkField DataTextField="descricao_linha" Text="descricao_linha" Visible="true" 
                    HeaderText="descricao_linha" DataNavigateUrlFormatString="~/modulos/colocar endere?o da pagina de detalhes .aspx?campoIndex={0}" 
                        DataNavigateUrlFields="//colocar o campo Index que fara a pesquisar" />  
...
using System.Data.SqlClient;
  ...
  {
                 protected static linhaDAO obj= null ;
     ...
     {
                                         obj.codigo_linha=int.Parse(txtcodigo_linha.Text);
...
/*--Campos Form
                                      <td >                   <p>codigo_linha</p>
                   <asp:TextBox ID="txtcodigo_linha" runat="server"  CssClass="numero" ></asp:TextBox>
                   </td>

                                      <td >                   <p>descricao_linha</p>
                   <asp:TextBox ID="txtdescricao_linha" runat="server" ></asp:TextBox>
                   </td>
