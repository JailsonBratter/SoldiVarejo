...
namespace visualSysWeb.dao
{
    public class CFOPDAO
    {
        ...
        public CFOPDAO() { }
        public CFOPDAO(String campoIndex, User usr)
        { //colocar campo index da tabela
            String sql = "Select * from  CFOP where CFOP =" + campoIndex;
        ...
        public void carregarDados(SqlDataReader rs)
        {
            if (rs.Read())
            {
                ...
                TIPO = int.Parse(rs["TIPO"].ToString());
            }
        }
        private void update()
        {
        ...
        public bool excluir()
        {
            String sql = "delete from CFOP  where cfop= " + CFOP; //colocar campo index
            Conexao.executarSql(sql);
            return true;
        }
