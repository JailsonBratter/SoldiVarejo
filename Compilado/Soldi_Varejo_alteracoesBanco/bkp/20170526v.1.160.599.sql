...
/****** Object:  UserDefinedFunction [dbo].[F_MASCARA_ID]    Script Date: 05/26/2017 10:04:57 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[F_MASCARA_ID]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[F_MASCARA_ID]
GO
...
CREATE FUNCTION [dbo].[F_MASCARA_ID](@CHAVE VARCHAR(44)) 
RETURNS VARCHAR(54)
AS
BEGIN
	IF LEN(RTRIM(LTRIM(ISNULL(@CHAVE,'')))) = 0
		SET @CHAVE = REPLICATE('0',44)

	DECLARE @Z	AS VARCHAR(55)
	DECLARE @Y	AS TINYINT
	...
	WHILE @Y < 45
		BEGIN
			SET @Z = @Z + CASE WHEN @Y = 1 THEN '' ELSE ' 'END + SUBSTRING(@CHAVE, @Y, 4) 
			SET @Y = @Y + 4
		END
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('natureza_operacao'))
begin
	alter table pedido alter column natureza_operacao varchar(20)
	alter table pedido alter column nome_transportadora varchar(20)
	alter table pedido alter column tipo_transporte varchar(3)
end
else
begin
		
	alter table pedido add natureza_operacao varchar(8)
	alter table pedido add nome_transportadora varchar(20)
	alter table pedido add tipo_transporte varchar(3)
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('Opt_Simples_nac'))
begin
	alter table cliente alter column Opt_Simples_nac tinyint
end
else
begin
		
    alter table cliente add Opt_Simples_nac tinyint
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('Cod_Trib_Inter'))
begin
	alter table Mercadoria alter column Cod_Trib_Inter Decimal(10,2)
end
else
begin
    alter table Mercadoria add Cod_Trib_Inter Decimal(10,2)
end 
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('Logo'))
begin
	alter table Filial alter column logo varbinary
end
else
begin
    alter table Filial add logo varbinary
end 
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('IDBarra'))
begin
	alter table NF alter column IDBarra varbinary(max)
	alter table NF alter column Producao_NFe TINYINT
	
end
else
begin
    alter table NF add IDBarra varbinary(max)
    alter table NF add Producao_NFe TINYINT
...
begin
	alter table NF alter column numero_protocolo varchar(50)
	
	
end
else
begin
    alter table NF add numero_protocolo varchar(50)
    
end 

go
-- Sp_help aliquota_imp_estado


/****** Object:  StoredProcedure [dbo].[sp_NF_DANFE]    Script Date: 05/29/2017 10:25:19 ******/
...
/****** Object:  StoredProcedure [dbo].[sp_NF_DANFE]    Script Date: 05/29/2017 10:25:20 ******/
SET ANSI_NULLS ON
GO
...
--sp_NF_DANFE 'MATRIZ', '240', '179', '1', 'N'
	...
	ELSE
		BEGIN
			...
			FROM         dbo.NF INNER JOIN
                         dbo.NF_Item ON dbo.NF.Filial = dbo.NF_Item.Filial AND dbo.NF.Cliente_Fornecedor = dbo.NF_Item.Cliente_Fornecedor AND dbo.NF.Codigo = dbo.NF_Item.Codigo INNER JOIN
                         dbo.Mercadoria ON dbo.Mercadoria.PLU = dbo.NF_Item.PLU INNER JOIN
                         dbo.Filial ON dbo.Filial.Filial = dbo.NF.Filial INNER JOIN
                         dbo.Natureza_operacao ON dbo.NF.Codigo_operacao = dbo.Natureza_operacao.Codigo_operacao LEFT OUTER JOIN
...
insert into Versoes_Atualizadas select 'Versão:1.160.599', getdate();
GO