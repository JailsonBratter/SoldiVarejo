...
as
begin 
	-- exec SP_REL_VENDAS_POR_GRUPO_NOVO 'MATRIZ','20180701','20181003','|MERCEARIA SECA|','|ACUCAR|','TODOS'
...
BEGIN 
		SELECT PLU,DESCRICAO,QTDE,VENDA,[%]  FROM (
			SELECT ORDEM = COD_DEPARTAMENTO +'000'
			 ...
			 UNION ALL 
			 SELECT ORDEM = COD_DEPARTAMENTO +'111'
					,'PLU'+PLU
					,DESCRICAO
					,QTDE = SUM(QTDE)
					,TOTAL = SUM(VALOR)
			...
			FROM #VendasGrupo
			GROUP BY PLU,DESCRICAO,COD_DEPARTAMENTO
			UNION ALL
			SELECT ORDEM = COD_DEPARTAMENTO +'999'
				,PLU ='|-SUB-|SUB-TOTAL'
				,DESCRICAO=''
				,QTDE = SUM(QTDE)
				,TOTAL  = SUM(VALOR)
...
insert into Versoes_Atualizadas select 'Versão:1.213.705', getdate();
GO
