...
begin
	alter table natureza_operacao alter column despesas_base tinyint
end
else
begin
	alter table natureza_operacao add despesas_base tinyint
end 

go 

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_rel_conta_a_receber]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_rel_conta_a_receber]
GO
...
--exec  sp_Rel_conta_a_Receber    @Filial='MATRIZ', @datade = '20190319',  @dataate = '20190319',  @tipo = 'emissao',  @Status = 'PREVISTO',  @valor = '',  @Cliente = '',  @cnpj = '03.667.884/0015-26',  @centrocusto = '' 
	...
	if(len(@grupo_empresa)>0)
	begin 
		set @Where = @where+ ' and c.grupo_empresa ='+@grupo_empresa
	end 
	--Monta Select
	set @string = 'select 
...
GO 
insert into Versoes_Atualizadas select 'Versão:1.252.793', getdate();