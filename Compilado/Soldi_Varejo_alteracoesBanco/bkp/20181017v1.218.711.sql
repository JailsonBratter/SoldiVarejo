...
if(OBJECT_ID('sp_NF_DANFE','P') is not null)
	drop procedure [sp_NF_DANFE]
go 
...
go 
if(OBJECT_ID('sp_rel_validade_produtos','P') is not null)
	drop procedure [sp_rel_validade_produtos]
go 


create procedure sp_rel_validade_produtos
	@filial varchar(20),
...
Select ordem = dep.descricao_departamento
	   ...
	   ,Validade = convert(varchar,data_validade,103)
...
insert into Versoes_Atualizadas select 'Versão:1.218.711', getdate();
GO
