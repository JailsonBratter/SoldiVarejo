...
begin
	alter table contrato_fornecedor_item alter column qtde_minima numeric(18,2)
end
else
begin
	alter table contrato_fornecedor_item add qtde_minima numeric(18,2)
end 
go 
...
/****** Object:  StoredProcedure [dbo].[sp_estoque_solicitacao_compra]    Script Date: 06/12/2017 09:41:50 ******/
...
begin
       ...
       --PRINT(@strSqlItens) 
        ...
        SET @strSqlItens= @strSqlItens+ '              inner join  Contrato_Fornecedor_Filial cff  on cfi.id_contrato = cff.id_contrato  AND CFI.plu = CFF.PLU '
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('data_Inicio'))
begin
		alter table Contrato_fornecedor alter column Descricao Varchar(40)
	    alter table Contrato_fornecedor alter column data_inicio DateTime
		alter table Contrato_fornecedor alter column data_base_reajuste DateTime
		alter table Contrato_fornecedor alter column forma_reajuste varchar(50)
		alter table Contrato_fornecedor alter column prazo_entrega int
	
	
end
else
begin
		alter table Contrato_fornecedor add Descricao Varchar(40)
		alter table Contrato_fornecedor add data_inicio DateTime
		alter table Contrato_fornecedor add data_base_reajuste DateTime
		alter table Contrato_fornecedor add forma_reajuste varchar(50)
		alter table Contrato_fornecedor add prazo_entrega int
		
end 
go 


-- sp_help contrato_fornecedor_item
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE UPPER(TABLE_NAME) = UPPER('Contrato_fornecedor_item') 
            AND  UPPER(COLUMN_NAME) = UPPER('und'))
begin
		alter table Contrato_fornecedor_item alter column und varchar(3)
end
else
begin
		alter table Contrato_fornecedor_item add und varchar(3)
end 
go 
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('id_contrato'))
begin
		alter table mercadoria_loja alter column id_contrato varchar(10)
end
else
begin
		alter table mercadoria_loja add id_contrato varchar(10)
end 
go 



IF not EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[solicitacao_pedidos]') AND type in (N'U'))
begin 
	create table solicitacao_pedidos
	(
		filial varchar(20),
		codigo_solicitacao varchar(20),
		pedido varchar(8)
	)
end 
go 

IF not EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[solicitacao_cotacao]') AND type in (N'U'))
begin 
...
insert into Versoes_Atualizadas select 'Versão:1.163.602', getdate();
GO