...
as
begin 
	   -- sp_Rel_Fin_PorOperador 'MATRIZ', '2021-08-06', '2021-08-06', 'TODOS', '', '', '', '', '', '', '', 'VALOR', 'DECRESCENTE', ''
	...
	INTO #VENDA01
	FROM #vendasTot
	GROUP BY PLU, DESCRICAO, GRUPO, SUBGRUPO, DEPARTAMENTO, FAMILIA   
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('indIntermed'))
begin
	alter table nf alter column indIntermed int
	alter table nf alter column intermedCnpj varchar(18)
	alter table nf alter column idCadIntTran varchar(60)
end
else
begin
	alter table nf add indIntermed int
	alter table nf add intermedCnpj varchar(18)
	alter table nf add idCadIntTran varchar(60)
end 
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('indIntermed'))
begin
	alter table pedido alter column indIntermed int
	alter table pedido alter column intermedCnpj varchar(18)
	alter table pedido alter column idCadIntTran varchar(60)
end
else
begin
	alter table pedido add indIntermed int
	alter table pedido add intermedCnpj varchar(18)
	alter table pedido add idCadIntTran varchar(60)
end 
go 
if OBJECT_ID('Intermediador') is null
begin
	CREATE TABLE [dbo].[Intermediador](
		[Codigo] [varchar](30) NULL,
		[CNPJ] [varchar](18) NULL,
		[Nome] [varchar](60) NULL,
		[CNPJPagamento] [varchar](18) null
	) ON [PRIMARY]
end 
GO
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE UPPER(TABLE_NAME) = UPPER('nf') 
            AND  UPPER(COLUMN_NAME) = UPPER('CNPJPagamento'))
begin
	alter table nf alter column CNPJPagamento VARCHAR(18)
end
else
begin
	ALTER TABLE NF ADD CNPJPagamento VARCHAR(18)
end 
go 
...
IF EXISTS( SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
            ...
            AND  UPPER(COLUMN_NAME) = UPPER('CNPJPagamento'))
begin
	alter table Pedido alter column CNPJPagamento VARCHAR(18)
end
else
begin
	ALTER TABLE Pedido ADD CNPJPagamento VARCHAR(18)
end 
go 
...
GO
insert into Versoes_Atualizadas select 'Versão:1.295.865', getdate();